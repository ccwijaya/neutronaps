...
class Sls_invoice extends CI_Controller {

	private $class_name = "sls_invoice";
	private $form_title = "Invoice";
	
	function __construct()
	{
		parent::__construct();
		$this->load->helper("custom");
		$this->load->model("m_sls_invoice");
	...
	function index()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$results = $this->m_sls_invoice->get_data();
	...
	function entry() {
		if($this->session->userdata('logged_in'))
		{
			...
			//$rs_cabang = $this->m_sls_invoice->get_cabang();
			//$rs_act_setting = $this->m_sls_invoice->get_act_setting();
			//$rs_segment = $this->m_sls_invoice->get_segment();
			$rs_ba = $this->m_sls_invoice->get_ba($id);
			$rs_customer = $this->m_sls_invoice->get_customer($id);
			$rs_detail_rr = $this->m_sls_invoice->get_detail_rr($id);
			$rs_cabang = $this->m_sls_invoice->get_cabang($id);
			$rs_produk_jasa = $this->m_sls_invoice->get_produk_jasa($id);
			$rs_produk_jasa_detail = $this->m_sls_invoice->get_produk_jasa_detail($id);
			$rs_sales = $this->m_sls_invoice->get_sales($id);
			...
			//$rs_satuan = $this->m_sls_invoice->get_satuan();
			// debug($rs_customer);
			//$rs_detail_pretelan = $this->m_sls_inquiry->get_detail_pretelan($id);
			
			if($id != ""){
				$results = $this->m_sls_invoice->get_data($id);
				// debug($rs_customer);
			}
			$data['results'] = $results;
	...
	function print_out() {
		if($this->session->userdata('logged_in')) {
			...
			$data["rs_data"] = $this->m_sls_invoice->get_data($id);
			//$data["rs_detail"] = $this->m_sls_invoice->get_detail($id);
			$data["rs_detail_print"] = $this->m_sls_invoice->get_detail_print($id);
			// debug($rs_data);
			//$data["results"] = $this->m_sls_invoice->get_data($id);
	...
	function get_pelanggan_by_id() {
		...
		$results = $this->m_sls_invoice->get_pelanggan_by_id($data);
		echo json_encode($results);
	
	}

	function get_ba_detail_by_id() {
		$data["id"] = $this->input->get('id');
		$results = $this->m_sls_invoice->get_ba_detail_by_id($data);
		echo json_encode($results);
	...
	function get_produk_jasa_detail_by_id() {
		$data["id"] = $this->input->get('id');
		$results = $this->m_sls_invoice->get_produk_jasa_detail_by_id($data);
		echo json_encode($results);
	...
	function simpan() {
		...
		if($data["id"]==""){
			...
			$data["no_bukti"] = "NMA-INV-" . date('d') . "-" . date('m') . date('y') . "-" . addzero(get_last_counter("NMA-BA" . date('Y-m-d')), 3);
			// YPH-FP-Q-23-0320-001
		}
		$data["id_ba"] = $this->input->post('id_ba');
		...
		// $data["no_container"] = $this->input->post('no_container');
		// $data["tujuan"] = $this->input->post('tujuan');
		// $data["jenis_container"] = $this->input->post('jenis_container');
		// $data["tempat_fumigasi"] = $this->input->post('tempat_fumigasi');
		// $data["waktu_pelepasan"] = $this->input->post('waktu_pelepasan');
		// $data["durasi_fumigasi"] = $this->input->post('durasi_fumigasi');
		// $data["fumigasi_digunakan"] = $this->input->post('fumigasi_digunakan');
		// $data["dosis_fumigasi"] = $this->input->post('dosis_fumigasi');
		//$data["status_approve"] = 0;
		

		// debug($this->input->post());
		$result = 0;
		$result = $this->m_sls_invoice->simpan_data($data, $data["id"]);	

		if($result!=0){
			
			$result = $this->m_sls_invoice->reset_data($data["id"]);
			//$result = $this->m_sls_invoice->reset_data_pic($data["id"]);
			...
			if($this->input->post('combo_produk_jasa_detail') != ""){
				...
				for($i=0;$i<count($a_combo_produk_jasa_detail);$i++){
					// debug(count());
					$data_detail["id_invoice"] = $data["id"];
					...
					$result = $this->m_sls_invoice->simpan_data_detail($data_detail);	
				}
			}
			...
			// 		$result = $this->m_sls_invoice->simpan_data_pic($data_pic);	
			// 	}
			// }
	...
	function delete_data() {
		...
		$result = $this->m_sls_invoice->delete_data($id);
		
		$result = $this->m_sls_invoice->reset_data($id);
		//$result = $this->m_sls_invoice->reset_data_pic($id);
