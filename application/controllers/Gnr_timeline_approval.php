...
class Gnr_timeline_approval extends CI_Controller
{
    private $class_name = "gnr_timeline_approval";
	private $form_title = "Gnr Timeline Approval";
	
	function __construct()
	{
		parent::__construct();
		$this->load->helper("custom");
		$this->load->model("m_gnr_timeline_approval");
	}
	...
	function index()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$results = $this->m_gnr_timeline_approval->get_data();
    ...
    function entry() {
		if($this->session->userdata('logged_in'))
		{
			...
			if($id != ""){
				$results = $this->m_gnr_timeline_approval->get_data($id);
			}
			$data['results'] = $results;
			// $data['rs_cabang'] = $rs_cabang;

			$rs_menu = $this->m_gnr_timeline_approval->get_menu();
	...
	function simpan() {	
		...
		// debug($data);
		$result = 0;
		$result = $this->m_gnr_timeline_approval->simpan_data($data, $data["id"]);	
	...
	function delete_data() {
		...
		$result = $this->m_gnr_timeline_approval->delete_data($id);		
