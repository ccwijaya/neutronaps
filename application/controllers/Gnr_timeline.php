...
class Gnr_timeline extends CI_Controller
{
    private $class_name = "gnr_timeline";
	private $form_title = "Gnr Timeline";
	
	function __construct()
	{
		parent::__construct();
		$this->load->helper("custom");
		$this->load->model("m_gnr_timeline");
	}
	...
	function index()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$results = $this->m_gnr_timeline->get_data();
    ...
    function entry() {
		if($this->session->userdata('logged_in'))
		{
			...
			$rs_menu = $this->m_gnr_timeline->get_menu();
			$data['rs_menu'] = $rs_menu;

			if($id != ""){
				$results = $this->m_gnr_timeline->get_data($id);
			}
			$data['results'] = $results;
	...
	function simpan() {	
		...
		// debug($data);
		$result = 0;
		$result = $this->m_gnr_timeline->simpan_data($data, $data["id"]);	
	...
	function delete_data() {
		...
		$result = $this->m_gnr_timeline->delete_data($id);		
