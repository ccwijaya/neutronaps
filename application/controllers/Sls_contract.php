...
class Sls_contract extends CI_Controller {

	private $class_name = "sls_contract";
	private $form_title = "Contract";
	
	function __construct()
	{
		parent::__construct();
		$this->load->helper("custom");
		$this->load->model("m_sls_contract");
	...
	function index()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$results = $this->m_sls_contract->get_data();
	...
	function entry() {
		if($this->session->userdata('logged_in'))
		{
			...
			//$rs_cabang = $this->m_sls_contract->get_cabang();
			//$rs_act_setting = $this->m_sls_contract->get_act_setting();
			//$rs_segment = $this->m_sls_contract->get_segment();
			$rs_rr = $this->m_sls_contract->get_rr();
			$rs_customer = $this->m_sls_contract->get_customer($id);
			$rs_detail_pnl = $this->m_sls_contract->get_detail_pnl($id);
			$rs_cabang = $this->m_sls_contract->get_cabang($id);
			$rs_sales = $this->m_sls_contract->get_sales($id);
			$rs_rute = $this->m_sls_contract->get_rute($id);
			$rs_moda = $this->m_sls_contract->get_moda($id);
			$rs_kategori_kirim = $this->m_sls_contract->get_kategori_kirim($id);
			$rs_jenis_muatan = $this->m_sls_contract->get_jenis_muatan($id);
			$rs_status_koreksi = $this->m_sls_contract->get_status_koreksi($id);
			$rs_pre_quote = $this->m_sls_contract->get_pre_quote($id);
			
			//debug($rs_product);
			

			//$rs_satuan = $this->m_sls_contract->get_satuan();
			// debug($rs_customer);
			//$rs_detail_pretelan = $this->m_sls_inquiry->get_detail_pretelan($id);
			
			if($id != ""){
				$results = $this->m_sls_contract->get_data($id);
				// debug($rs_customer);
			}
			$data['results'] = $results;
	...
	function print_out() {
		if($this->session->userdata('logged_in')) {
			...
			$data["rs_data"] = $this->m_sls_contract->get_data($id);
			$data["rs_detail"] = $this->m_sls_contract->get_detail($id);
			$data["rs_detail_print"] = $this->m_sls_contract->get_detail_print($id);
			// debug($rs_data);
			//$data["results"] = $this->m_sls_contract->get_data($id);
	...
	function get_pelanggan_by_id() {
		...
		$results = $this->m_sls_contract->get_pelanggan_by_id($data);
	...
	function get_rute_by_id() {
		$data["id"] = $this->input->get('id');
		$results = $this->m_sls_contract->get_rute_by_id($data);
		echo json_encode($results);
	...
	function get_kategori_kirim_by_id() {
		$data["id"] = $this->input->get('id');
		$results = $this->m_sls_contract->get_kategori_kirim_by_id($data);
		echo json_encode($results);
	...
	function get_pq_detail_by_id() {
		$data["id"] = $this->input->get('id');
		$results = $this->m_sls_contract->get_pq_detail_by_id($data);
		echo json_encode($results);
	...
	// 	$results = $this->m_sls_contract->get_segment_by_id_db_customer($data);
	// 	echo json_encode($results);
	// }
	...
	function simpan() {
		...
		if($data["no_contract"]==""){
			$data["no_contract"] = "PBL-CTR-" . date('d') . "-" . date('m') . date('y') . "-" . addzero(get_last_counter("CONTRACT-" . date('Y-m-d')), 3);
		}
		//$data["no_pre_quote"] = "PBL-PQ" . date('d') . "-" . date('m') . date('y') . "-" . addzero(get_last_counter("PQ-" . date('Y-m-d')), 3);
		//$data["id_paf"] = $this->input->post('id_paf');
		$data["tanggal_contract"] = db_date($this->input->post('tanggal_contract'));
		$data["cvp"] = $this->input->post('cvp');
		$data["is_contract"] = 1;
		...
		// debug($this->input->post());
		$result = 0;
		$result = $this->m_sls_contract->simpan_data($data, $data["id"]);	

		if($result!=0){
			
			//$result = $this->m_sls_contract->reset_data($data["id"]);
			//$result = $this->m_sls_contract->reset_data_pic($data["id"]);
			...
			//if($this->input->post('combo_rute_moda') != ""){
				...
				//for($i=0;$i<count($a_combo_rute_moda);$i++){
					...
					//$result = $this->m_sls_contract->simpan_data_detail($data_detail);	
				//}
			//}
	...
	function delete_data() {
		...
		$result = $this->m_sls_contract->delete_data($id);
		
		//$result = $this->m_sls_contract->reset_data($id);
		//$result = $this->m_sls_contract->reset_data_pic($id);
