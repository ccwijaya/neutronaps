...
class Sls_quotation_unlock extends CI_Controller {

	private $class_name = "sls_quotation_unlock";
	private $form_title = "Quotation Unlock";
	...
	function simpan() {
		...
		$data["waktu_approved_q"] = date("Y-m-d H:i:s");
		$data["nama_approved_q"] = $this->input->post('nama_approved_q');
		...
		if($result!=0){
			...
			$this->session->set_flashdata('success', 'Unlock');  
		} else {
