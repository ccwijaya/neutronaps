...
class Cabang extends CI_Controller {
	...
	function index()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$results = $this->m_cabang->get_data();
	...
	function entry() {
		if($this->session->userdata('logged_in'))
		{
			...
			//$rs_cabang = $this->m_umk->get_cabang();
			// $rs_kategori = $this->m_unit->get_kategori();
			
			if($id != ""){
				$results = $this->m_cabang->get_data($id);
			}
			$data['results'] = $results;
	...
	function simpan() {	
		...
		// debug($data);
		$result = 0;
		$result = $this->m_cabang->simpan_data($data, $data["id"]);	
	...
	function delete_data() {
		...
		$result = $this->m_cabang->delete_data($id);		
