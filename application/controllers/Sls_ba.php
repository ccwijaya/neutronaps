...
class Sls_ba extends CI_Controller {

	private $class_name = "sls_ba";
	private $form_title = "Berita Acara";
	
	function __construct()
	{
		parent::__construct();
		$this->load->helper("custom");
		$this->load->model("m_sls_ba");
	...
	function index()
	{
		if($this->session->userdata('logged_in'))
		{
			...
			$results = $this->m_sls_ba->get_data();
	...
	function entry() {
		if($this->session->userdata('logged_in'))
		{
			...
			//$rs_cabang = $this->m_sls_ba->get_cabang();
			//$rs_act_setting = $this->m_sls_ba->get_act_setting();
			//$rs_segment = $this->m_sls_ba->get_segment();
			$rs_po_customer = $this->m_sls_ba->get_po_customer($id);
			$rs_customer = $this->m_sls_ba->get_customer($id);
			$rs_detail_rr = $this->m_sls_ba->get_detail_rr($id);
			$rs_cabang = $this->m_sls_ba->get_cabang($id);
			$rs_produk_jasa = $this->m_sls_ba->get_produk_jasa($id);
			$rs_produk_jasa_detail = $this->m_sls_ba->get_produk_jasa_detail($id);
			$rs_sales = $this->m_sls_ba->get_sales($id);
			...
			//$rs_satuan = $this->m_sls_ba->get_satuan();
			// debug($rs_customer);
			//$rs_detail_pretelan = $this->m_sls_inquiry->get_detail_pretelan($id);
			
			if($id != ""){
				$results = $this->m_sls_ba->get_data($id);
				// debug($rs_customer);
			}
			$data['results'] = $results;
			$data['rs_cabang'] = $rs_cabang;
			$data['rs_po_customer'] = $rs_po_customer;
	...
	function print_out() {
		if($this->session->userdata('logged_in')) {
			...
			$data["rs_data"] = $this->m_sls_ba->get_data($id);
			//$data["rs_detail"] = $this->m_sls_ba->get_detail($id);
			$data["rs_detail_print"] = $this->m_sls_ba->get_detail_print($id);
			// debug($rs_data);
			//$data["results"] = $this->m_sls_ba->get_data($id);
	...
	function get_pelanggan_by_id() {
		...
		$results = $this->m_sls_ba->get_pelanggan_by_id($data);
		echo json_encode($results);
	
	}

	function get_po_detail_by_id() {
		$data["id"] = $this->input->get('id');
		$results = $this->m_sls_ba->get_po_detail_by_id($data);
		echo json_encode($results);
	...
	function get_produk_jasa_detail_by_id() {
		$data["id"] = $this->input->get('id');
		$results = $this->m_sls_ba->get_produk_jasa_detail_by_id($data);
		echo json_encode($results);
	...
	function simpan() {
		...
		if($data["id"]==""){
			...
			$data["no_bukti"] = "NMA-BA-" . date('d') . "-" . date('m') . date('y') . "-" . addzero(get_last_counter("NMA-BA" . date('Y-m-d')), 3);
			// YPH-FP-Q-23-0320-001
		}
		$data["id_po_customer"] = $this->input->post('id_po_customer');
		...
		// debug($this->input->post());
		$result = 0;
		$result = $this->m_sls_ba->simpan_data($data, $data["id"]);	

		if($result!=0){
			
			$result = $this->m_sls_ba->reset_data($data["id"]);
			//$result = $this->m_sls_ba->reset_data_pic($data["id"]);
			...
			if($this->input->post('combo_produk_jasa_detail') != ""){
				...
				for($i=0;$i<count($a_combo_produk_jasa_detail);$i++){
					...
					$result = $this->m_sls_ba->simpan_data_detail($data_detail);	
				}
			}
			...
			// 		$result = $this->m_sls_ba->simpan_data_pic($data_pic);	
			// 	}
			// }
	...
	function delete_data() {
		...
		$result = $this->m_sls_ba->delete_data($id);
		
		$result = $this->m_sls_ba->reset_data($id);
		//$result = $this->m_sls_ba->reset_data_pic($id);
