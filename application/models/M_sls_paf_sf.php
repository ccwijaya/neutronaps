<?php
class M_sls_paf_sf extends CI_Model {
	...
	public function get_data($id=""){
		...
		$this->db->select ( 'a.*, b.pic, b.nama_customer, c.nama_cabang' );
		...
		$this->db->where("a.is_sf", 1);
		$this->db->where("a.is_hold_paf", 0);
		$this->db->where("a.is_dist_paf",NULL);
	...
	public function get_data_hold($id=""){
		$session_data = $this->session->userdata('logged_in');		
		$id_cabang = $session_data['id_cabang'];
		$is_cat_customer = $session_data['is_cat_customer'];
		...
		$this->db->join('pa_project_summary d', 'a.id = d.id_paf', 'left');	
		$this->db->where("a.is_sf", 1);
		$this->db->where("a.is_pre_project", 2);
		$this->db->where("a.is_hold_paf", 1);
	...
	public function get_data_approval($id=""){
		$session_data = $this->session->userdata('logged_in');		
		$id_cabang = $session_data['id_cabang'];
		$is_cat_customer = $session_data['is_cat_customer'];
		...
		$this->db->where("a.is_sf", 1);	
		$this->db->where("a.is_pre_project", 2);
		$this->db->where("a.is_edit_access", 1);
	...
	public function get_data2_hold($search, $limit="", $start="", $id=""){
		$session_data = $this->session->userdata('logged_in');		
		$id_cabang = $session_data['id_cabang'];
		$id_sales = $session_data['id_sales'];
		$nama_user = $session_data['nama_user'];
	...
	public function get_data2_approval($search, $limit="", $start="", $id=""){
		$session_data = $this->session->userdata('logged_in');		
		$id_cabang = $session_data['id_cabang'];
		$id_sales = $session_data['id_sales'];
		$nama_user = $session_data['nama_user'];
	...
	public function get_rute(){
		...
		$this->db->from("view_rute_sf a");
		$this->db->join('moda b', 'b.id = a.id_moda', 'left');	
		//$this->db->where("a.nama_kategori", "REGULAR | OPTIONAL TOL");
		// if($id_cabang!=0){REGULAR | OPTIONAL TOL
		//$this->db->where("a.total_km<>", NULL);
		$this->db->where("a.loading<>", "");
		// }
	...
	public function get_detail_tarif_kirim($id){
		$this->db->select ( 'a.*, b.rute' );
		$this->db->from("sls_paf_tarif_kirim a");
		$this->db->join('view_rute_sf b', 'b.id = a.rute_tarif', 'left');
	...
	public function simpan_rute($data, &$id = ""){
		...
		$result = $this->db->insert("rute_sf", $data);
		$id = $this->db->insert_id();
