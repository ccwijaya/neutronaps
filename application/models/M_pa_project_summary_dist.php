<?php
class M_pa_project_summary_dist extends CI_Model {
	...
	public function get_detail_ps($id){
		$this->db->select ( "a.*, CONCAT(b.kode_dist,' | ',b.dop_start,' - ',b.dop_end,' | ',b.unit,' ', b.tipe_box) as rute, b.loading, b.tipe_box, c.unit" );
		$this->db->from("pa_project_summary_detail a");
		$this->db->join('view_dist_rute b', 'a.id_rute = b.id', 'left');
	...
	public function get_ps_detail_by_id($param){
		...
		$this->db->from("sls_paf a");
		$this->db->join('customer b', 'a.id_customer = b.id', 'left');
		$this->db->join('view_dist_rute_paf_date_plus3 c', 'a.id = c.id_paf', 'left');
		//$this->db->where("c.post_date <=3 ");
		$this->db->where("a.status_pai", "VERIFIED");
	...
	public function simpan_data_detail_matrix($data, &$id = ""){
		...
		$result = $this->db->insert("pa_project_summary_matrix", $data);
		$id = $this->db->insert_id();
	...
	public function reset_data_matrix($id){
		$this->db->where('id_ps', $id);
        $result = $this->db->delete("pa_project_summary_matrix");
	...
	public function simpan_data_detail_matrix_pbl($data, &$id = ""){
		...
		$result = $this->db->insert("pa_project_summary_matrix_pbl", $data);
		$id = $this->db->insert_id();
	...
	public function reset_data_matrix_pbl($id){
		$this->db->where('id_ps', $id);
        $result = $this->db->delete("pa_project_summary_matrix_pbl");
