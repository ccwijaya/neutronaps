<?php
class M_sls_quotation_dist extends CI_Model {
	...
	public function get_detail_pnl($id){
		$this->db->select ( "a.*, CONCAT(b.kode_dist,' | ',b.dop_start,' - ',b.dop_end,' | ',b.unit,' ', b.tipe_box,' | ', b.nama_kategori) as rute" );
	...
	public function get_detail_print($id){
		$this->db->select ( 'a.*, c.kode_dist, c.dop_start, c.dop_end, c.loading, c.unloading, f.unit, c.tipe_box, c.satuan_konversi, e.nama_user, e.jabatan, g.nama_customer, h.nama_sales, h.keterangan' );
		$this->db->from("sls_quotation_detail a");
		$this->db->join('sls_quotation b', 'a.id_profit = b.id', 'left');
		$this->db->join('view_dist_rute c', 'a.id_rute = c.id', 'left');
		$this->db->join('web_user e', 'e.id = b.create_user', 'left');
	...
	public function get_pre_quote($id=""){
		...
		$this->db->select ( 'a.*, b.nama_customer' );
		$this->db->from("view_dist_profit_margin a");
		$this->db->join('customer b', 'a.id_customer = b.id', 'left');
		$this->db->where("a.is_pre_project<>", 1);
