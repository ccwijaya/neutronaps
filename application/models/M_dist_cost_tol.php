<?php
class M_dist_cost_tol extends CI_Model {
	private $main_table = "view_dist_cost_tol";
	...
	public function get_data2($search, $limit="", $start="", $id=""){
		...
		extract($search);
		$this->db->select ( 'a.*, b.kode_dist, d.unit, b.loading, b.unloading, c.nama_kategori, b.rute');
		$this->db->from($this->main_table . " a");
		$this->db->join('view_dist_rute b', 'a.id_rute = b.id', 'left');
		$this->db->join('kategori_kirim c', 'a.id_kategori_kirim = c.id', 'left');
		$this->db->join('moda d', 'b.id_moda = d.id', 'left');
	...
	public function get_detail_tol($id){
		$this->db->select ( 'a.*' );
		$this->db->from("dist_cost_tol_detail a");
	...
	public function simpan_data($data, &$id = ""){
		...
		if($id!=""){		
			...
			$result = $this->db->update("dist_cost_tol", $data);
		} else {
			$result = $this->db->insert("dist_cost_tol", $data);
	...
	public function simpan_data_detail($data, &$id = ""){
		...
		$result = $this->db->insert("dist_cost_tol_detail", $data);
		$id = $this->db->insert_id();
	...
	public function reset_data($id){
        ...
        $result = $this->db->delete("dist_cost_tol_detail");
