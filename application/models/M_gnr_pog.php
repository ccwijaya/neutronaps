<?php
class M_gnr_pog extends CI_Model {
	...
	public function get_data($id=""){
		...
		$this->db->where("a.status_verify_pog", 1);
		$this->db->where("a.is_received", 1);
		//$this->db->where("a.no_bukti_pog<>","" );
		if($id!=""){
	...
	public function get_detail_ps($id){
		$this->db->select ( 'a.*, b.loading, b.rute, c.unit, b.tipe_box' );
	...
	public function get_detail($id){
		...
		$this->db->from("pa_project_summary_detail a");
		$this->db->join('pa_project_summary b', 'a.id_rr = b.id', 'left');
	...
	public function get_grouping_v(){
		$session_data = $this->session->userdata('logged_in');		
		$id_cabang = $session_data['id_cabang'];
			
		$this->db->select ( 'a.*, b.vendor, b.grade, c.nama_customer ' );
		$this->db->from("vm_grouping_v a");
		$this->db->join('vm_grouping_v_grading b', 'b.id_grouping = a.id', 'left');
		$this->db->join('customer c', 'a.id_customer = c.id', 'left');
		//$this->db->where("a.status_approved",1);
		// $this->db->where("a.status_ops <> ''");
		// if($id_cabang!=0){
		// 	$this->db->where("a.id_cabang", $id_cabang);
		// }
	...
	public function get_ps_detail_by_id($param){
		...
		$this->db->select ( 'a.*, b.nama_customer, c.rute_tarif, c.id_tipe_box_tarif, c.tarif_kirim, c.satuan_tarif, 
							...
							c.ton_minggu_pbl, c.trip_minggu_pbl, c.min_trip_hari_pbl, c.ton_minggu_vend, c.trip_minggu_vend, c.min_trip_hari_vend, c.post_date' );
		$this->db->from("sls_paf a");
		$this->db->join('customer b', 'a.id_customer = b.id', 'left');
	...
	public function get_grv_detail_by_id($param){
		extract($param);
		$session_data = $this->session->userdata('logged_in');		
		$id_cabang = $session_data['id_cabang'];
			
		$this->db->select ( 'a.*, b.vendor, b.grade, c.rute, ' );
		$this->db->from("vm_grouping_v a");
		$this->db->join('vm_grouping_v_grading b', 'b.id_grouping = a.id', 'left');
		$this->db->join('view_rute c', 'b.id_rute_grv = c.id', 'left');
		$this->db->where("b.grade<>", "D");
		//$this->db->where("b.status_approved_detail", 1);
		$this->db->where("a.id", $id);
	...
	public function get_detail_matrix_pbl($id){
		...
		$this->db->where("b.jabatan_mtx_pbl<>", "Sales");
		$this->db->where("b.jabatan_mtx_pbl<>", "SPV Sales");
		$this->db->where("b.jabatan_mtx_pbl<>", "Sales SPV");
		$this->db->where("b.jabatan_mtx_pbl<>", "Sales Supervisor");
		$this->db->where("a.id", $id);
		if($id_cabang!=0){
			$this->db->where("a.id_cabang", $id_cabang);
		}
