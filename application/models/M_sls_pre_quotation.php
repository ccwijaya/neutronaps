<?php
class M_sls_pre_quotation extends CI_Model {
	private $main_table = "profit_margin";
	...
	public function get_data($id=""){
		...
		if($session_data['id_level']<2){
			$this->db->where("a.nama_admin_pq", $session_data['nama_user']);
		}
	...
	public function get_detail_pnl($id){
		...
		$this->db->from("profit_margin_detail a");
	...
	public function get_detail($id){
		$this->db->select ( 'a.*, c.loading, c.unloading, f.unit, c.tipe_box, e.nama_user, e.ttd, b.ttd_admin, b.nama_approved, b.ttd_approved, g.nama_customer' );
		$this->db->from("profit_margin_detail a");
		$this->db->join('profit_margin b', 'a.id_profit = b.id', 'left');
	...
	public function get_detail_print($id){
		$this->db->select ( 'a.*, c.loading, c.unloading, f.unit, c.tipe_box, e.nama_user, e.ttd, b.ttd_admin, b.nama_admin_pq, b.jabatan_admin_pq, b.nama_approved, b.jabatan_approve, b.ttd_approved, g.nama_customer' );
		$this->db->from("profit_margin_detail a");
		$this->db->join('profit_margin b', 'a.id_profit = b.id', 'left');
		$this->db->join('view_rute c', 'a.id_rute = c.id', 'left');
		$this->db->join('web_user e', 'e.id = b.create_user', 'left');
	...
	public function get_re_detail_by_id(){
		$session_data = $this->session->userdata('logged_in');		
		$id_cabang = $session_data['id_cabang'];
		...
		$this->db->join('sls_rate_request_detail c', 'a.id = c.id_rr', 'left');
	...
	public function simpan_data_detail($data, &$id = ""){
		...
		$result = $this->db->insert("profit_margin_detail", $data);
		$id = $this->db->insert_id();
	...
	public function reset_data($id){
        ...
        $result = $this->db->delete("profit_margin_detail");
