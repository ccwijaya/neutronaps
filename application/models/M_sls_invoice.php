<?php
class M_sls_invoice extends CI_Model {
	private $main_table = "sls_invoice";
	...
	public function get_data($id=""){
		...
		$this->db->select ( 'a.*, b.nama_cabang, c.nama_customer, c.alamat, c.pic, c.kota, c.kode_pos, d.nama_sales, f.nama_user, g.nama_produk, DATE_ADD(a.tanggal , INTERVAL 30 DAY) as jatuh_tempo');
	...
	public function get_detail_rr($id){
		$this->db->select ( "a.*" );
		$this->db->from("sls_invoice_detail a");
	...
	public function get_detail($id){
		...
		$this->db->from("sls_invoice_detail a");
		$this->db->join('sls_invoice b', 'a.id_invoice = b.id', 'left');
	...
	public function simpan_data_detail($data, &$id = ""){
		...
		$result = $this->db->insert("sls_invoice_detail", $data);
		$id = $this->db->insert_id();
	...
	public function reset_data($id){
        ...
        $result = $this->db->delete("sls_invoice_detail");
