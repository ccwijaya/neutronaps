<?php
class M_sls_quotation_dist_non extends CI_Model {
	...
	public function get_data($id=""){
		...
		$this->db->where("a.is_distribution", 1);
	...
	public function get_detail_pnl($id){
		$this->db->select ( "a.*, b.loading, CONCAT(b.kode_dist,' | ',b.dop_start,' - ',b.dop_end,' | ',b.unit,' ', b.tipe_box,' | ', b.nama_kategori) as rute" );
		$this->db->from("sls_quotation_detail a");
		$this->db->join('view_dist_rute_moda b', 'a.id_rute = b.id', 'left');
	...
	public function get_detail($id){
		$this->db->select ( 'a.*, c.kode_dist, f.unit, c.tipe_box, e.nama_user, g.nama_customer' );
		$this->db->from("sls_quotation_detail a");
	...
	public function get_detail_print($id){
		$this->db->select ( 'a.*, c.kode_dist, f.unit, c.dop_start, c.dop_end, c.tipe_box, c.satuan_konversi, e.nama_user, e.jabatan, g.nama_customer' );
		$this->db->from("sls_quotation_detail a");
		$this->db->join('sls_quotation b', 'a.id_profit = b.id', 'left');
		$this->db->join('view_dist_rute c', 'a.id_rute = c.id', 'left');
		$this->db->join('web_user e', 'e.id = b.create_user', 'left');
	...
	public function get_pq_detail_by_id($param){
		...
		$this->db->select ( 'a.*, c.id_rute, c.tarif_sales, c.tarif_nego, c.tarif_approved, tipe_box' );
		$this->db->from("dist_profit_margin a");
		$this->db->join('dist_profit_margin_detail c', 'a.id = c.id_profit', 'left');
		$this->db->join('view_dist_rute d', 'c.id_rute = d.id', 'left');
		$this->db->where("a.id", $id);
	...
	public function get_rute(){
		...
		$this->db->select ( "a.id, a.id_cost_tol, CONCAT(a.kode_dist,' | ',a.dop_start,' - ',a.dop_end,' | ',a.unit,' ', a.tipe_box,' | ', a.nama_kategori) as nama" );
